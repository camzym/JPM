...
let bang = m.key.id
await Skyzo.sendMessage(m.chat, {text: `@${m.sender.split("@")[0]} Maaf Kamu Akan Saya Keluarkan Dari Grup Ini Karna Admin/Owner Bot Menyalakan Fitur *Antilink* Grup Lain!`, contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: "https://files.catbox.moe/ksutgs.jpg", title: "｢ LINK GRUP DETECTED ｣", previewType: "PHOTO"}}}, {quoted: m})
...
let delet = m.key.participant
let bang = m.key.id
await Skyzo.sendMessage(m.chat, {text: `@${m.sender.split("@")[0]} Maaf Pesan Kamu Saya Hapus Karna Admin/Owner Bot Menyalakan Fitur *Antilink* Grup Lain!`, contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: "https://files.catbox.moe/yd74gh.jpg", title: "｢ LINK GRUP DETECTED ｣", previewType: "PHOTO"}}}, {quoted: m})
await Skyzo.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
}
}}

const qdoc = {key : {participant : '0@s.whatsapp.net', ...(m.chat ? { remoteJid: `status@broadcast` } : {}) },message: {documentMessage: {title: 'Whatsapp Bot By LixxKAV',jpegThumbnail: ""}}}

const qloc = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `Whatsapp Jpm By LixxKAV`,jpegThumbnail: ""}}}

const qloc2 = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `Wa Bot By LixxKAV`,jpegThumbnail: ""}}}
...
const qbug = {key: {remoteJid: 'status@broadcast', fromMe: false, participant: '0@s.whatsapp.net'}, message: {listResponseMessage: {title: `𝐋𝐢𝐱𝐱𝐊𝐀𝐕 𝐇𝐀𝐖𝐊𝜩`
}}}
...
const textnya = `    *Mode Bot :* ${Skyzo.public ? "Public" : "Self"}
 ...
 ┅═❏ *SC BOT 𝐌𝐀𝐑𝐆𝐀 𝐇𝐀𝐖𝐊𝜩* ❏═┅‎‎
 友 𝐂𝐫𝐞𝐚𝐭𝐨𝐫 : *𝐋𝐢𝐱𝐱𝐊𝐀𝐕 𝜩*
 友 𝐍𝐮𝐦𝐛𝐞𝐫 : *6287843241782*
 友 𝐍𝐚𝐦𝐚 𝐒𝐜𝐫𝐢𝐩𝐭 : *𝐁𝐨𝐭 𝐏𝐮𝐬𝐡 𝐊𝐨𝐧𝐭𝐚𝐤 𝐕𝟐*
▭▬▭▬▭▬▭▬▭▬▭▬▭▬
    ...
    *⏣ 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐒𝐚𝐥𝐮𝐫𝐚𝐧:*
    https://whatsapp.com/channel/0029Vay7kE8LdQegSNnlkW1N
...
break
case "vps": {
let teks = `*List Harga VPS By LixxKAV*
*(ISP Digital Ocean)*
...
break
case "domain": {
let teks = `*List Harga Domain By LixxKAV*
...
break
case "panel": {
const teks = `*LIST BOTZ BY LixxKAV*
...
break
case "owner": {
Skyzo.sendContact(m.chat, [owner], "Telfon Atau Vc = Blok", m)
}
break
case "antilink": {
...
} else return m.reply(example('62878XXX'))}
break
...
if (!isGroup) return m.reply(msg.group)
if (!args[0]) return m.reply(example("62878XXX"))
...
if (!isOwner) return m.reply(msg.owner)
if (!text) return m.reply(example("62878XX,jumlahbug"))
if (!text.split(",")) return m.reply(example("62878XX,jumlahbug"))
...
const teks = `*TRANSAKSI DONE BY LixxKAV*
...
newsletterName: `Testimoni By LixxKAV`
}}}, {quoted: qloc})
total += 1
} catch {}
await sleep(global.delayjpm)
...
let messageText = "*🌐 LIST SERVER PANEL BOT LixxKAV🔥*\n\n"
...
if (!isOwner) return m.reply(msg.owner)
let teks = `*｢ LIST ALL PAYMENT LixxKAV ｣*
